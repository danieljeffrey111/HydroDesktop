...
{
    public partial class frmGBMPrediction : DockContent
    {
        ...
        private dynamic gbm_model;


        public frmGBMPrediction()
        {
            InitializeComponent();
        ...
        private void btnImportObs_Click(object sender, EventArgs e)
        {
            ...
            obsColumns.Add(gbm_model.target, gbm_model.target);
            ...
            string target = gbm_model.target;
            for (int indx = 0; indx < dgvObs.RowCount; indx++)
                if (Convert.ToDouble(dgvObs.Rows[indx].Cells[target].Value) > gbm_model.regulatory_threshold)
        ...
        private void btnMakePredictions_Click(object sender, EventArgs e)
        {
            ...
            dynamic predictions = ipyInterface.Predict(gbm_model, tblPrediction);
